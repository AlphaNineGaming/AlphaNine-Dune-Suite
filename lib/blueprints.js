...
function normalizeBlueprint(blueprint, fileName = "") {
  ...
  const normalizedInstances = instances.map((row, index) => {
    ...
    return {
      ...
      stability: Boolean(row.provides_stability)
    };
  });
...
function createBlueprintService({ query, audit = () => {} }) {
  ...
  async function list(playerRef = "") {
    ...
    if (ref) {
      await query(`
        with ${playerTargetCte(ref)}, repaired as (
          update dune.items i
          set stats = jsonb_set(
            i.stats,
            '{FBuildingBlueprintItemStats,1,PlayerBlueprintId}',
            to_jsonb(('!!bbp#' || bb.id::text)::text),
            true
          )
          from dune.building_blueprints bb, dune.inventories inv
          where bb.item_id = i.id
            and inv.id = i.inventory_id
            and coalesce(bb.player_id, inv.actor_id) = (select pawn_id from target limit 1)
            and i.stats #>> '{FBuildingBlueprintItemStats,1,PlayerBlueprintId}'
                is distinct from ('!!bbp#' || bb.id::text)
          returning i.id
        )
        select count(*)::text from repaired`, 30000);
    }
  ...
  async function importBlueprint(playerRef, input, fileName = "") {
    ...
    const output = await query(`
      ...
      blueprint_key as materialized (
        select nextval('dune.building_blueprints_id_seq'::regclass)::bigint as id
      ),
      inserted_item as (
        ...
        select inventory.id, 1, inventory_usage.next_position, 'BuildingBlueprint_CopyDevice', 0,
          jsonb_build_object(
            ...
            'FBuildingBlueprintItemStats', jsonb_build_array('[]'::jsonb, jsonb_build_object('PlayerBlueprintId', '!!bbp#' || blueprint_key.id::text, 'BuildingBlueprintName', ${sqlString(name)})),
            'FItemStackAndDurabilityStats', jsonb_build_array('[]'::jsonb, jsonb_build_object('DecayedMaxDurability', 0.0))
          )
        from inventory, inventory_usage, blueprint_key
      ...
      inserted_blueprint as (
        insert into dune.building_blueprints (id, item_id, player_id, building_blueprint_map)
        select blueprint_key.id, inserted_item.id, null::bigint, '' from inserted_item, blueprint_key
  ...
  return { capabilities, list, importBlueprint, exportBlueprint };
}
