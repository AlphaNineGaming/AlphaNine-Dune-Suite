  ...
  async function importBlueprint(playerRef, input, fileName = "") {
    ...
    const output = await query(`
      ...
      inserted_item as (
        ...
        select inventory.id, 1, inventory_usage.next_position, 'BuildingBlueprint_CopyDevice', 0,
          jsonb_build_object(
            ...
            'FBuildingBlueprintItemStats', jsonb_build_array('[]'::jsonb, jsonb_build_object('PlayerBlueprintId', '!!bbp#0', 'BuildingBlueprintName', ${sqlString(name)}, 'PlayerBaseBackupId', '{}'::jsonb)),
      ...
      inserted_blueprint as (
        ...
        select inserted_item.id, target.pawn_id, '' from inserted_item cross join target
        returning id, item_id
      ),
      updated_item as (
        update dune.items i set stats = jsonb_build_object(
          'FCustomizationStats', jsonb_build_array('[]'::jsonb, '{}'::jsonb),
          'FBuildingBlueprintItemStats', jsonb_build_array('[]'::jsonb, jsonb_build_object('PlayerBlueprintId', '!!bbp#' || bp.id::text, 'BuildingBlueprintName', ${sqlString(name)}, 'PlayerBaseBackupId', '{}'::jsonb)),
          'FItemStackAndDurabilityStats', jsonb_build_array('[]'::jsonb, jsonb_build_object('DecayedMaxDurability', 0.0))
