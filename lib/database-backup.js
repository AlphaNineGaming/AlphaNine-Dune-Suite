...
function resolveVendorArtifactIdentity(operation, reportedPath) {
  ...
  const references = artifactReferences(operation);
  if (candidate && !path.posix.isAbsolute(candidate)) throw new Error("Vendor backup reported a non-absolute artifact path.");
  if (candidate) {
    ...
    return { path: candidate, fileName: candidateName, operationReference: matching[0], source: "command-output+database-operation" };
  }
  const absoluteArchives = [...new Set(references.map((reference) => String(reference).replace(/\\/g, "/")).filter((reference) => (
    path.posix.isAbsolute(reference) && /\.(?:backup|bgbackup|tar|zip)$/i.test(path.posix.basename(reference))
  )))];
  if (absoluteArchives.length !== 1) throw new Error("The successful DatabaseOperation did not expose one unambiguous absolute backup artifact path.");
  const resolved = absoluteArchives[0];
  return { path: resolved, fileName: path.posix.basename(resolved), operationReference: resolved, source: "database-operation" };
}
...
function validateFullBackupToc(tocText, expectedInventory, options = {}) {
  ...
  if (unexpectedSchema.length) throw new Error(`PostgreSQL archive TOC contains unexpected schema entries (${descriptorCountText(unexpectedSchema)}).`);
  const tableDefinitions = new Set(entries.filter((entry) => entry.descriptor === "TABLE").map((entry) => entry.definition));
  const requiredAlphaTables = Array.isArray(options.requiredAlphaTables) ? [...options.requiredAlphaTables] : [];
