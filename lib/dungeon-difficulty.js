...
function buildApplySql({ playerId: playerIdValue, dungeonId: dungeonIdValue, targetSelectableDifficulty, writeMethod = "function" }) {
  if (!["function", "direct"].includes(writeMethod)) throw new Error("Unsupported dungeon write method.");
  const playerId = normalizePlayerId(playerIdValue);
  const dungeonId = normalizeDungeonId(dungeonIdValue);
  const target = normalizeTargetDifficulty(targetSelectableDifficulty);
  const requiredDifficulty = requiredCompletionDifficulty(target);
...
begin
  if not exists (
    ...
    ${writeMethod === "function" ? `perform dune.record_dungeon_completion(${dungeon}, ${requiredDifficulty}, ${SYNTHETIC_DURATION_MS}, array[${playerId}]::bigint[]);` : `with created as (
      insert into dune.dungeon_completion (dungeon_id, difficulty, duration_ms, players_num)
      values (${dungeon}, ${requiredDifficulty}, ${SYNTHETIC_DURATION_MS}, 1)
      returning completion_id
    )
    insert into dune.dungeon_completion_players (player_id, completion_id)
    select ${playerId}, completion_id from created;`}
  end if;
end
$dungeon_unlock$;` : "";
  return `
...
lock table dune.dungeon_completion, dune.dungeon_completion_players in share row exclusive mode;
...
returning dcp.completion_id
)

delete from dune.dungeon_completion dc
where dc.dungeon_id = ${dungeon}
  and dc.completion_id in (select completion_id from removed)
  and not exists (
    select 1 from dune.dungeon_completion_players dcp where dcp.completion_id = dc.completion_id and dcp.player_id <> ${playerId}
  );
${insert}
