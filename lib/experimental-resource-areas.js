...
const PROVENANCE = Object.freeze({
  ...
  orientationEvidence: "Independent orientation evidence supports source image top equals world maximum Y.",
  nonIncorporation: "No third-party project code, resource markers, external CDN files, map tiles, icons, heatmap PNGs, or other external project assets were incorporated."
});
...
function findRepakExecutable(preferred = "", appDir = process.cwd()) {
  ...
  throw new Error("The packaged resource-area extraction helper was not found or is unsupported. Reinstall the current AlphaNine Dune Suite release. No Funcom assets are downloaded or bundled.");
}
...
function verifyOfflineDecompressor(repakExe) {
  ...
  if (!existingFile(decompressor)) {
    throw new Error("The packaged offline resource-area decompressor is missing. Reinstall the current AlphaNine Dune Suite release. The Suite will not download a decompressor at runtime.");
  }
  if (fileSha256(decompressor).toLowerCase() !== OPEN_SOURCE_DECOMPRESSOR_SHA256) {
    throw new Error("The packaged offline resource-area decompressor failed its integrity check. Reinstall the current AlphaNine Dune Suite release.");
  }
  return decompressor;
}
...
function compatiblePakCopy(source, destination, mountPoint) {
  ...
  try {
    ...
    if (footer.readUInt32LE(0) !== 0x5a6f12e1 || footer.readInt32LE(4) !== 11) throw new Error(`${path.basename(source)} footer is not the supported Dune build format.`);
    ...
    if (relative < 0) throw new Error(`${path.basename(source)} primary index could not be located.`);
...
function packageNames(asset) {
  ...
  if (u32() !== 0x9e2a83c1 || i32() !== -8) throw new Error("Asset has an unsupported Unreal package header.");
