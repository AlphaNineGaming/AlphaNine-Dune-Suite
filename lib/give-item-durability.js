...
function durabilityEligibility(item = null) {
  ...
  if (["schematics", "resources", "consumables", "augment", "customization", "customizations", "decorations", "misc", "items", "clothes", "vehicle"].includes(category)) {
    return {
      applicable: false,
      kind: "non-durable",
  ...
  if (["garment", "armor", "armour"].includes(category)) {
...
function durabilityExpectation(item, requested) {
  ...
  return {
    ...
    display: eligibility.applicable
      ? (applied ? "200 current durability" : "Eligible; option not selected")
...
function itemStatsForGrant(expectation) {
  const durability = expectation?.applied ? { CurrentDurability: SET_DURABILITY_VALUE } : {};
  return {
    FCustomizationStats: [[], {}],
...
function classifyDurabilityReadBack(expectation, observed = {}) {
  ...
  if (expectation?.applied) {
    ...
    if (numeric !== found || exact !== found) return { ok: false, status: "durability-mismatch", message: "Durability read-back did not remain the JSON number 200 on every granted item row." };
  } else if (expectation?.applicable === false && present > 0) {
    return { ok: false, status: "fabricated-durability", message: "A non-durable or unselected grant unexpectedly contains fabricated CurrentDurability data." };
  }
  return { ok: true, status: "database-verified", message: expectation?.applied
    ? "Every granted item row retains exactly 200 current durability."
    : (expectation?.applicable === false ? "Durability not applicable; no CurrentDurability data was written." : "Durability was not requested for this eligible item.") };
}
...
function buildDurabilityInvestigationSql(templateSqlLiteral) {
  return `
    ...
    select s.data_type, s.udt_name, c.stats_type, p.current_rows, p.numeric_rows,
           p.selected_current_rows, p.selected_numeric_rows
