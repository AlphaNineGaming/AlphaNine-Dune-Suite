...
function canonicalItemCategory(value) {
  ...
  const categories = {
    ...
    clothes: "Clothing", clothing: "Clothing", garment: "Clothing", garments: "Clothing",
    consumable: "Consumables", consumables: "Consumables",
    contract: "Contracts", contracts: "Contracts",
    customization: "Customization", customizations: "Customization",
    emote: "Emotes", emotes: "Emotes",
    gadget: "Gadgets", gadgets: "Gadgets",
    itemmod: "Item Mods", itemmods: "Item Mods",
    meleeweapon: "Weapons", meleeweapons: "Weapons", rangedweapon: "Weapons", rangedweapons: "Weapons", rangeweapon: "Weapons", rangeweapons: "Weapons", weapon: "Weapons", weapons: "Weapons",
    misc: "Miscellaneous Equipment", miscequipment: "Miscellaneous Equipment", miscellaneousequipment: "Miscellaneous Equipment",
    placeable: "Placeables", placeables: "Placeables",
    resource: "Resources", resources: "Resources",
    schematic: "Schematics", schematics: "Schematics",
    vehicle: "Vehicles", vehicles: "Vehicles", vehiclebase: "Vehicles", vehicleextra: "Vehicles"
  };
  return categories[compact] || raw;
}

function installedGameCatalogItem(item = {}) {
  return normalizeItem({ ...item, _classificationOverride: true }, "installed-game");
}
...
function createItemCatalogProvider(options = {}) {
  ...
  const installedGameCatalogPaths = (Array.isArray(options.installedGameCatalogPaths)
    ? options.installedGameCatalogPaths
    : [options.installedGameCatalogPath]).filter(Boolean).map((pathname) => path.resolve(pathname));
  ...
  function snapshot({ refresh = false } = {}) {
    ...
    const installedGameSources = installedGameCatalogPaths.map((pathname) => ({ pathname, ...readOptionalItems(pathname, "installed-game", installedGameCatalogItem) }));
    const installedGameItems = mergeByPriority(installedGameSources.map((source) => source.items));
    ...
    const items = mergeByPriority([installedGameItems, bundled.items, manager.items, learned.items, legacy.items]);
    lastSnapshot = {
      ok: true,
      items,
      generatedAt: new Date().toISOString(),
      report: {
        sourcePriority: ["installed-game", "bundled", "locally-stored-server-discovered", "live-read-only-discovery", "raw-id-fallback"],
        installedGameCatalogPaths,
        installedGameCatalogPresent: installedGameSources.some((source) => source.present),
        installedGameCatalogItems: installedGameItems.length,
        installedGameCatalogSchematics: installedGameItems.filter((item) => item.category === "Schematics").length,
        installedGameCatalogError: installedGameSources.map((source) => source.error).filter(Boolean).join("; "),
        bundledCatalogPath,
