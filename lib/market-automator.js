...
function createMarketAutomator(options = {}) {
  ...
  const startupSuppressed = options.startupSuppressed === true;
  ...
  if (!startupSuppressed) fs.mkdirSync(dataDir, { recursive: true });
  ...
  function load() {
    try {
      if (fs.existsSync(configPath)) {
        ...
        if (!startupSuppressed && JSON.stringify(stored) !== JSON.stringify(config)) writeConfig(config);
      }
    } catch (error) {
      counters.errors += 1;
      if (!startupSuppressed) log("error", "Configuration load failed", error.message);
    }
    return config;
  }
  ...
  load();
  const timer = startupSuppressed ? null : setInterval(scheduledCheck, 30000);
