...
function concisePodReason(item, containers, podWarnings) {
  const severe = containers.find((container) => SEVERE_CONTAINER_REASONS.has(container.reason))
    || containers.find((container) => SEVERE_CONTAINER_REASONS.has(container.lastTerminationReason));
  ...
  if (relevantEvent) return `${relevantEvent.reason}: ${relevantEvent.message}`;
  ...
  if (notReady) return `${notReady.name}: ${notReady.reason || notReady.state}`;
  return "";
}
...
function parsePods(items, warnings, podMetrics, nowMs) {
  ...
  return items.map((item) => {
    ...
    const severe = containers.some((container) => SEVERE_CONTAINER_REASONS.has(container.reason) || SEVERE_CONTAINER_REASONS.has(container.lastTerminationReason))
      || podWarnings.some((event) => event.recent && /FailedScheduling|Unhealthy|BackOff|FailedMount|ImagePull/i.test(event.reason));
    let state = "Unknown";
    if (phase === "Succeeded") state = "Healthy";
    else if (phase === "Failed") state = "Unhealthy";
    else if (phase === "Unknown") state = "Unknown";
    else if (severe) state = "Unhealthy";
    else if (phase === "Pending" || readyContainers < totalContainers) state = "Degraded";
    else if (phase === "Running") state = "Healthy";
...
function parseNamespaces(items, pods, nowMs) {
  ...
  return [...known.entries()].map(([name, item]) => {
    ...
    return {
      name,
      state: unhealthy ? "Unhealthy" : (degraded ? "Degraded" : "Healthy"),
...
function buildServerHealthReport(input = {}) {
  ...
  if (vmOffline || (!sshOk && connectivity.hyperv.state === "Offline")) {
    ...
    const criticalUnhealthy = [
      ...
      ...recentUnhealthyPods.slice(0, 3).map((pod) => `${pod.namespace}/${pod.name}: ${pod.reason || pod.phase}`),
    ...
    if (criticalUnhealthy.length) {
      ...
      const degradedReasons = [
        ...
        ...recentWarnings.slice(0, 3).map((event) => `${event.reason}: ${event.message}`)
      ].filter(Boolean);
