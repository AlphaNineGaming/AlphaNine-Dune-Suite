...
function parseNamespaces(items, pods, nowMs) {
  ...
  return [...known.entries()].map(([name, item]) => {
    ...
    return {
      name,
      state: activeUnhealthy ? "Unhealthy" : (completedFailures || degraded ? "Degraded" : "Healthy"),
...
function buildServerHealthReport(input = {}) {
  ...
  const recentFailedPods = recentUnhealthyPods.filter((pod) => pod.phase === "Failed");
  ...
  if (vmOffline || (!sshOk && connectivity.hyperv.state === "Offline")) {
    ...
    if (criticalUnhealthy.length) {
      ...
      const degradedReasons = [
        ...
        ...recentFailedPods.slice(0, 3).map((pod) => `${pod.namespace}/${pod.name}: recent pod failure (${pod.reason || pod.phase}).`)
      ].filter(Boolean);
