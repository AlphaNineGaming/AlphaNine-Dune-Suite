...
function serviceGroup({ label, namespace, podPattern, servicePattern, pods, services, endpoints, workloads, database, inventoryAvailable = true, optional = false }) {
  ...
  if (database && database.ok !== true) {
    state = database.status === "unavailable" ? "Unknown" : "Unhealthy";
    message = redactHealthText(database.error || database.message || "Database connectivity check failed.");
  } else if (!inventoryAvailable) {
    state = "Unknown";
    message = "Pod inventory unavailable; readiness could not be checked.";
...
function buildServerHealthReport(input = {}) {
  ...
  const kubernetesResponded = Object.values(parsed).some(result => result.ok);
  ...
  const postgres = serviceGroup({
    ...
    endpoints,
    workloads,
    inventoryAvailable: parsed.pods.ok,
  ...
  } else if (!parsed.pods.ok) {
    duneState = "Unknown";
    duneMessage = "Pod inventory unavailable; server readiness could not be checked.";
  ...
  const connectivity = {
    ...
    ssh: {
      state: sshOk ? "Healthy" : (vmOffline ? "Offline" : "Unknown"),
      status: sshOk ? "Connected" : "Unconfirmed",
    ...
    kubernetes: {
      state: kubernetesConnected ? (nodes.every((node) => node.ready) ? "Healthy" : "Unhealthy") : "Unknown",
      status: kubernetesResponded ? "Connected" : "Unconfirmed",
      message: kubernetesConnected ? `${nodes.length} node(s) discovered.` : (kubernetesResponded ? "API responded, but node readiness could not be checked: " : "API check unavailable: ") + parsed.nodes.error,
  ...
  if (vmOffline || (!sshOk && connectivity.hyperv.state === "Offline")) {
  ...
  } else if (!kubernetesResponded) {
    overallState = "Unknown";
    reasons.push("Kubernetes checks did not return usable data; cluster health is unconfirmed.");
  } else {
    ...
    if (criticalUnhealthy.length) {
      ...
      const degradedReasons = [
        ...["nodes", "workloads", "namespaces", "networking", "pvcs", "events"].filter(key => !parsed[key].ok).map(key => parsed[key].error),
        ...(postgres.state === "Unknown" ? [postgres.message] : []),
        ...(!parsed.pods.ok ? ["Pod inventory unavailable: " + parsed.pods.error] : []),
  ...
  return {
    ...
    readOnly: true,
    inventoryAvailability: Object.fromEntries(Object.entries(parsed).map(([key, result]) => [key, result.ok])),
    ...
    summary: {
      ...
      nodes: parsed.nodes.ok ? nodes.length : null,
      podInventoryAvailable: parsed.pods.ok,
      pods: parsed.pods.ok ? pods.length : null,
      containers: parsed.pods.ok ? pods.reduce((sum, pod) => sum + pod.containers.length, 0) : null,
      readyContainers: parsed.pods.ok ? pods.reduce((sum, pod) => sum + pod.readyContainers, 0) : null,
      restarts: parsed.pods.ok ? pods.reduce((sum, pod) => sum + pod.restarts, 0) : null,
      workloads: workloads.length,
      unhealthyWorkloads: unhealthyWorkloads.length,
      pvcs: pvcs.length,
      recentWarnings: parsed.events.ok ? recentWarnings.length : null
    },
    ...
    services: {
      ...
      dune: {
        ...
        state: duneState,
        message: duneMessage,
        podCount: parsed.pods.ok ? dunePods.length : null,
        readyPodCount: parsed.pods.ok ? duneReady : null,
