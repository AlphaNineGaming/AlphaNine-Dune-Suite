...
function buildInstallCommand({ config, scriptSource, appVersion }) {
  ...
  const steps = [
    ...
    ["cron-runtime", "sudo -n ps -eo comm= 2>/dev/null | grep -E '^crond$' >/dev/null"],
...
function buildStatusCommand({ displayOnly = false } = {}) {
  ...
  const processGate = displayOnly ? `[ "$helper_process_refs" -ge 0 ]` : `[ "$helper_process_refs" = 0 ]`;
  ...
  return [
    ...
    `if [ \"$filesystem_evidence_ok\" = true ] && [ \"$cron_evidence_ok\" = true ] && [ \"$alternate_cron_evidence_ok\" = true ] && [ \"$kubernetes_evidence_ok\" = true ] && [ \"$process_evidence_ok\" = true ] && [ \"$openrc_evidence_ok\" = true ] && [ \"$alternate_cron_refs\" = 0 ] && [ \"$kubernetes_cronjob_refs\" = 0 ] && ${processGate} && [ \"$openrc_restart_refs\" = 0 ]; then`,
    `  if [ \"$directory_exists\" = true ] && [ \"$script_exists\" = true ] && [ \"$config_exists\" = true ] && [ \"$begin_count\" = 1 ] && [ \"$end_count\" = 1 ] && [ \"$path_refs\" = 1 ]; then ${displayOnly ? `runtime_status=$(${shellQuote(VM_SCHEDULER_SCRIPT)} status) && printf %s "$runtime_status" | jq --argjson active "$helper_process_refs" ' . + {activeHelperCount: $active, workloadActive: ($active > 0)}'` : `${shellQuote(VM_SCHEDULER_SCRIPT)} status`}`,
