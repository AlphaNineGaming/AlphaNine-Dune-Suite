...
sed "s|@NODE_BINARY@|$NODE_BIN|g" "$INSTALL_DIR/linux/alphanine-dune-suite.service" > /etc/systemd/system/alphanine-dune-suite.service
...
#!/usr/bin/env sh
exec runuser -u alphanine-suite -- env ALPHANINE_DATA_DIR=/var/lib/alphanine-dune-suite "$NODE_BIN" /opt/alphanine-dune-suite/scripts/set-remote-password.js "\$@"
EOF
