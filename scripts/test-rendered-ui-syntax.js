...
const electronMainSource = fs.readFileSync(path.join(__dirname, "..", "electron", "main.js"), "utf8");
...
assert.equal(packageManifest.marketBotRuntimeVersion, "1.0.102", "Suite and Market Bot runtime release versions are not pinned to the expected compatibility pair.");
...
(async () => {
  try {
    ...
    const serverControlSection = html.slice(html.indexOf('<section id="server"'), html.indexOf('<section id="scheduler"'));
    assert(!serverControlSection.includes('id="liveUserGamePanel"'), "Live UserGame.ini panel still appears in Server Control.");
