...
function progressionPlayerLookupValue(p){return String(p?.player_controller_id||p?.character_id||p?.player_pawn_id||p?.character_name||p?.name||p?.funcom_id||p?.account_id||p?.id||"");}
