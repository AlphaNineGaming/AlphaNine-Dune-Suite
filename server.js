    ...
    <section id="give" class="view">
      <div class="give-layout">
        ...
        <div class="panel pad give-catalog">
          ...
          <div id="sellerVariantFilterNote" class="warning mt hidden">Seller Variants shows non-MTX customization variants that appear safe for Give Item. DLC and MTX variants are intentionally hidden.</div>
...
function isSellerVariantItem(item){const id=String(item?.id||"").trim().toLowerCase();const type=String(item?.type||"").trim().toLowerCase();const subtype=String(item?.subtype||"").trim().toLowerCase();const category=String(item?.category||"").trim().toLowerCase();if(!id)return false;if(id.startsWith("mtx_"))return false;return category==="customization"&&(type==="variant"||subtype==="variant"||id.includes("variant"));}
