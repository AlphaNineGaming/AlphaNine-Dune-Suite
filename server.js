...
async function confirmMapDeployWhenOnline(map,replicas){if(Number(replicas)<=0)return true;let status=null;try{status=await getJson("/api/status",{timeoutMs:8000});}catch(error){return await appConfirm("Confirm map start","Could not confirm whether the Battlegroup is online before starting "+map+".\n\nStatus check error: "+betterError(error)+"\n\nContinue anyway?","Start Map","Cancel");}const mapped=status.topServerStatus||status.serverStatus||status.runtimeTransport?.serverStatusMapped||mapServerSummary(status);if(!mapped?.online)return true;const selected=status.selectedBattlegroup?((status.selectedBattlegroup.title||"Title not found")+" / "+status.selectedBattlegroup.namespace+" / "+status.selectedBattlegroup.name):"Selected Battlegroup";return await appConfirm("Server is online","The Battlegroup is currently online.\n\nMap: "+map+"\nReplicas: "+replicas+"\nTarget: "+selected+"\n\nStarting or scaling a map while online can affect active players. Continue?","Start Map","Cancel");}
