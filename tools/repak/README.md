...
AlphaNine packages repak with an integrity-checked, open-source `ooz`
compatibility DLL for offline decompression. The DLL is not the proprietary
RAD Game Tools library. Its complete corresponding source and build
instructions are in `ooz-source/`. The Suite refuses to run extraction if the
DLL is missing or changed, preventing repak's network fallback.
